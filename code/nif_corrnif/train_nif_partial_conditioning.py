"""
Tier-2 Fix C: partial-realization conditioning training script.

Adapted from train_nif_hardlabel_v2.py's run-dir/checkpoint/resume/logging
conventions, but the data pipeline is different in one essential way: each
training sample carries a `realization_id` column that must be used to look
up that realization's own observed-region occupancy map (from a separate
`observed_maps.npz`) before every forward pass — see
build_levelA_helical_nif_partial_conditioning_dataset.py for how the dataset
is built and models/nif_partial_conditioning.py for the model
(PartialConditionedNIF.forward(x, obs_map) takes the map as a second,
explicit argument).

Gate: APPROVE_PARTIAL_CONDITIONING_NIF_TRAINING
"""
...
MODEL_PATH    = PINN_NIF_ROOT / "pinn_training" / "models" / "nif_partial_conditioning.py"

KNOWN_GATES = {"APPROVE_PARTIAL_CONDITIONING_NIF_TRAINING"}
...
def load_model_module():
    spec = importlib.util.spec_from_file_location("nif_partial_conditioning", MODEL_PATH)
...
def make_run_dir(cfg: dict, resume_dir: str | None) -> Path:
    ...
    return (resolve_project_path(cfg.get("output_root", "pinn_training/runs"), PINN_NIF_ROOT)
            / f"{cfg.get('run_name', 'nif_partial_conditioning')}_{stamp}")
...
def load_split(path: Path) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Returns (x_features[x,y,z,alpha_norm], realization_id[int64], label)."""
    arr = np.load(path)["data"].astype(np.float32)
    x_feat = torch.tensor(arr[:, :4])
    rid    = torch.tensor(arr[:, 4]).long()
    label  = torch.tensor(arr[:, 5])
    return x_feat, rid, label


def load_observed_maps(dataset_dir: Path) -> torch.Tensor:
    arr = np.load(dataset_dir / "observed_maps.npz")["observed_maps"].astype(np.float32)
    return torch.tensor(arr)  # (R, H, W)


def evaluate(model, x, rid, y, obs_maps, batch_size, device) -> dict:
    model.eval()
    losses = []
    ...
    with torch.no_grad():
        for s in range(0, len(x), batch_size):
            ...
            ridb = rid[s:s + batch_size]
            obsb = obs_maps[ridb].to(device)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train partial-conditioning NIF (Fix C). Permission-gated.")
    ...
    x_train, rid_train, y_train = load_split(dataset / "train_samples.npz")
    x_val,   rid_val,   y_val   = load_split(dataset / "val_samples.npz")
    obs_maps = load_observed_maps(dataset)

    print(f"Dataset: {dataset.relative_to(PROJECT_ROOT)}")
    print(f"  train={len(x_train):,}  val={len(x_val):,}  realizations={len(obs_maps)}")
    print(f"  global pos_frac (train): {y_train.mean():.4f}")

    mod   = load_model_module()
    model = mod.build_partial_conditioning_model(cfg).to(device)
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Model: PartialConditionedNIF  params={n_params:,}  device={device}")

    bce = torch.nn.BCELoss()
    loss_fn = lambda pred, tgt: bce(pred, tgt)
    ...
    obs_maps_device = obs_maps.to(device)  # small (R,50,50); keep resident on device
    ...
    with (log_csv.open("a", encoding="utf-8", newline="") as fcsv,
        ...
        for epoch in range(start_epoch, epochs + 1):
            ...
            for s in range(0, len(perm), batch):
                ...
                ridb = rid_train[idx]
                obsb = obs_maps_device[ridb]
                yb   = y_train[idx].to(device).clamp(0.0, 1.0)
                opt.zero_grad(set_to_none=True)
                loss = loss_fn(model(xb, obsb), yb)
                loss.backward()
                opt.step()
                losses.append(float(loss.item()))
            sched.step()

            metrics    = evaluate(model, x_val, rid_val, y_val, obs_maps_device, batch, device)
            ...
            print(f"  ep={epoch:>4}  train_bce={train_loss:.4f}  val_bce={metrics['loss']:.4f}"
    ...
    (run_dir / "training_summary.md").write_text(
        f"# NIF Partial-Conditioning Training Summary\n\n"
