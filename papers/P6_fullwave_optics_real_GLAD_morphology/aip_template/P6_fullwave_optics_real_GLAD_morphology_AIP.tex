...
shell thickness taken from a prior thickness sweep. The underlying ballistic
morphology itself is purely metallic-Cu geometry with no surface mobility
(companion paper); the oxide shell is applied afterward as a static optical
layer, not as a growth process. The companion atomistic-anchor paper's
room-temperature Cu adatom diffusion length ($L_D\approx\SIrange{20}{30}{nm}$)
is a bare-metal-surface estimate; adatom mobility on an oxidising or
already-oxidised surface is a distinct transport problem not addressed by
either paper, and the two should not be assumed to share the same mobility
regime without a dedicated calibration.
...
value at 500\,nm by a known, technique-dependent margin.
