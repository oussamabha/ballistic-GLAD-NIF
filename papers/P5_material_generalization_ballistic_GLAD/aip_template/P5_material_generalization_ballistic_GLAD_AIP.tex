...
More specifically, that companion literature-corpus study finds, in a
predictor-isolation test restricted to elemental metals ($n=9$), that
covalent atomic radius is the one candidate predictor of the tangent-rule
offset that approaches statistical significance (+36.7\% LOMO-CV RMSE,
$p=0.205$), an order of magnitude closer to significance than any thermal
or kinetic predictor tested ($p\geq0.7$ throughout, including its own
$T_s/T_m$ test, $p=0.89$). The present simulation's own radius correlation
($r=0.57$, $p=0.032$, $n=14$) independently converges on the same channel:
material identity enters through the atomic length scale, not through a
thermal-mobility proxy -- a conclusion strengthened, not just paralleled, by
arising from two disjoint data sources and analysis methods.
