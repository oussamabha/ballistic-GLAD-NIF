...
\begin{abstract}
% [2026-09-07, per O.B.H.A. instruction -- abstract re-led on the invertibility result; human author to finalise wording]
Glancing-angle deposition (GLAD) turns a single control knob---the
deposition angle $\dang$---into a wide family of nanostructured thin-film
morphologies used in optical coatings, sensors and catalysis. Relating a
finished film back to the angle that made it, however, still relies on
slow, destructive cross-sectional imaging. Here we show that a purely
geometric ballistic growth model, containing no adjustable physical
parameters, is faithful enough to be \emph{inverted}: gradient descent
through a compact neural surrogate of the model recovers the deposition
angle behind a measured two-point structural descriptor, converging to the
same optimum as an exhaustive search (to $0.01^{\circ}$) from any starting
guess.
% [2026-09-08, per O.B.H.A. instruction -- honest noise-propagated precision from LENS1 WP1; human author to finalise phrasing]
Propagating a realistic descriptor-measurement noise through the local
sensitivity of the inverse map gives an achievable angular precision of
$\approx1^{\circ}$ near grazing incidence, degrading to a few degrees at
$\dang\sim65^{\circ}$; the inverse map is, counter-intuitively,
best-conditioned exactly where direct column-tilt morphometry fails.
Beyond that,
what has not been established is how much of a film's internal structure a
simplified, physics-based computer model can actually predict from
$\dang$ alone. We address this with a
...
$0.002^{\circ}$ --- an optimiser-convergence figure, not a measurement
uncertainty.
% [2026-09-08, per O.B.H.A. instruction -- add the noise-propagated precision; human author to finalise]
Propagating an assumed per-bin descriptor noise $\sigma_h$ through the
autodiff sensitivity $\lVert\partial h_g/\partial\dang\rVert$ of the frozen
model gives a first-order achievable precision
$\sigma_{\dang}\simeq\sigma_h/\lVert\partial h_g/\partial\dang\rVert$; for
$\sigma_h=0.01$ this is $\approx0.9^{\circ}$ at $\dang=89^{\circ}$ rising to
$\approx2.4^{\circ}$ at $\dang=65^{\circ}$, because
$\lVert\partial h_g/\partial\dang\rVert$ increases monotonically toward
grazing incidence (a $2.6\times$ range over $\dang=63$--$89^{\circ}$).
An illustrative optical calculation (treating the film's
