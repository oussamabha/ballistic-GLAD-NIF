...
\usepackage{siunitx}
\usepackage{orcidlink} % 2026-09-09: corresponding-author ORCID, standard CTAN
                        % package, available on Overleaf by default
...
% [2026-09-09 -- author order set to: PhD student first, co-supervisor (Paris
%  internship host) second, principal supervisor last -- standard convention when this
%  is NOT a cotutelle (confirmed: PhD registered in Tunisia, INSP Paris stays are
%  internships, cotutelle was refused). Previously ordered as if a cotutelle with two
%  co-directeurs of equal standing, which this is not. Human author to confirm this
%  matches what both supervisors expect before submission.]
% [ORCID -- corresponding author (O.B.H.A.) needs a registered ORCID iD before
%  submission: npj Computational Materials (Nature Partner Journals) requires one at
%  the submission portal for the corresponding author, no exceptions. Not a manuscript
%  LaTeX field -- entered at the portal, not typeset here. Register free at
%  https://orcid.org/register (2 min, needs only an email). RESOLVED 2026-09-09 --
%  ORCID 0009-0001-5823-7655 provided by the author, added below via \orcidlink.]
...
\affiliation{Laboratoire de Photovolta\"ique et Mat\'eriaux
  Semi-conducteurs (LPMS), \'Ecole Nationale d'Ing\'enieurs de Tunis (ENIT),
  Universit\'e de Tunis El Manar (UTM), Tunis, Tunisia}
% [2026-09-09 -- Nicolas Martin (FEMTO-ST) removed from the author list: he has not
%  contributed to this manuscript. Re-add only if he genuinely contributes after being
%  asked (see EXPERIMENTALIST_COLLABORATION_ASK_20260908.md), not before.]
...
cross-sectional imaging. We show that a purely geometric ballistic growth
model, with no physical parameter fitted to the target data, is faithful
enough to be \emph{inverted} -- no prior
...
gives an achievable precision $\sigma_{\dang}\approx0.9$--$2.4^{\circ}$ near
...
column-tilt morphometry fails. Injecting realistic descriptor noise
directly, and querying the surrogate on its far-field correlation tail
alone, leave recovery within this same range, confirming the limit is
set by angular sensitivity rather than noise level or descriptor choice. Across eight helical-rotation Cu
configurations ($\dang=60$--$89^{\circ}$, $h=\SI{300}{nm}$), void fraction
increases monotonically from $87.9\%$ to $98.7\%$ with $\dang$, tracking
...
\citep{alvarez2014beyondtangent}.  A geometric model with no physical
parameter fitted to this comparison is therefore tested against the
\emph{trend} in tilt with $\dang$ and against
...
$+\SI{20.7}{pp}$ against published optical void fractions
...
openness-trend curve, $\bsP = 87.9\%$, is drawn from a checkpoint passing
all three checks.
...
$\bsP = \SI{87.9}{\%}$ at $\dang = 60^{\circ}$ to $\bsP = \SI{98.7}{\%}$ at
...
\begin{figure}[htbp]
  ...
  \caption{%
    ...
    angles completed to date, reaching $98.7\%$ at $\dang = 89^{\circ}$;
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    60 & $4.120\times10^{7}$ & 87.9 & 4120 \\
    65 & $3.804\times10^{7}$ & 88.9 & 3804 \\
    70 & $3.443\times10^{7}$ & 89.9 & 3443 \\
    75 & $2.960\times10^{7}$ & 91.3 & 2960 \\
    80 & $2.335\times10^{7}$ & 93.2 & 2335 \\
    85 & $1.468\times10^{7}$ & 95.7 & 1468 \\
    87 & $1.010\times10^{7}$ & 97.0 & 1010 \\  % [2026-09-09] $\bsP$ recomputed at $R=\SI{0.128}{nm}$ (the paper's own adopted radius), resolving the prior reduced-reference-radius mismatch flagged 2026-09-07; verified against the same GRIDFIX-confirmed $N$ values.
    89 & $4.540\times10^{6}$ & 98.7 & 454 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  \item \textit{Normalised-height convergence.}  When height is
    ...
    height series at fixed $\dang$ (available in a separate fixed-angle
    height-series simulation set, not analysed here) would be required to attribute the collapse to growth
...
\begin{figure}[htbp]
  ...
  \caption{%
    ...
    $+20.7$~percentage points at $\dang = 85^{\circ}$.
...
At $\dang = 85^{\circ}$, the ballistic simulation yields $\bsP = 95.7\%$,
...
comparison.  The $+20.7\,\mathrm{pp}$ difference observed at $\dang=85^{\circ}$
...
reported here ($87.9$--$98.7\%$ over $\dang = 60$--$89^{\circ}$): the two
...
We verify this reproducibility directly using three independent
seeds at $\dang = 80^{\circ}$ and $85^{\circ}$ ($R=\SI{0.128}{nm}$,
GRIDFIX/independent-re-validation campaigns; recomputed 2026-09-09 after
completing the third seed at $80^{\circ}$, previously available only at
$R=\SI{0.1}{nm}$), computing $h_g(r)$ separately for each seed.  To avoid
the optimistic bias of comparing each seed only against a mean that
contains it, we report \emph{independent} estimators: the mean pairwise
(seed-to-seed) root-mean-square deviation is $0.0074$ at $80^{\circ}$ and
$0.0105$ at $85^{\circ}$, and the leave-one-seed-out deviation (each seed
versus the mean of the other two) is $0.0064$ and $0.0092$, respectively
(maximum pointwise standard deviation $0.0082$ at both angles).  The
self-inclusive deviation used in earlier drafts ($0.0032$--$0.0076$)
...
and about $1.05$--$1.55\times$ above, the independent seed-to-seed scatter:
...
optimum. With only five interpolation folds and two extrapolation folds
(bounded by the seven available training angles), these means are
indicative of a real interpolation/extrapolation asymmetry, not converged
uncertainty estimates in their own right; the qualitative conclusion --
interpolation is reliable, extrapolation is markedly worse -- is the
robust part of this result, not the exact mean values. The original
$\dang=72^{\circ}$ demonstration ($0.82^{\circ}$ error)
...
\textit{Robustness to measurement noise and to descriptor choice.} The LOO
sweep above uses noiseless, full-resolution target curves; a referee could
reasonably ask whether the recovered angle survives realistic measurement
noise, and whether the inversion depends on the exact way the correlation
curve is sampled rather than on genuine angular information in it. We test
both directly on the frozen, already-trained model, with no retraining.
Injecting calibrated Gaussian noise into each target curve at the level
actually observed between simulation seeds ($\sigma=0.008$) and at five
times that level ($\sigma=0.05$) leaves the recovered angle essentially
unchanged: mean interpolation/extrapolation error moves from
$4.16^{\circ}/4.96^{\circ}$ (noiseless) to $4.27^{\circ}/4.87^{\circ}$
($\sigma=0.008$) to $4.35^{\circ}/6.28^{\circ}$ ($\sigma=0.05$). The
residual error at $\sigma=0$ already accounts for essentially all of the
error at realistic noise levels, indicating that recovery precision is set
by the descriptor's local angular sensitivity, not by measurement noise per
se. Separately, querying the model on only the correlation curve's
far-field tail ($r$ above the sampled median) or on a threefold-coarser
radial grid tracks the full-curve recovery at every angle
($3.30^{\circ}/0.25^{\circ}$ and $4.60^{\circ}/4.63^{\circ}$
interpolation/extrapolation respectively) -- the tail-only variant is in
fact the \emph{best}-performing of the three at the two extrapolation
points, indicating the far-field correlation tail alone carries most of
the angle-discriminating information used by the inversion, rather than
the inversion being an artefact of the specific full-curve binning chosen
for this paper. Both checks are, like the LOO sweep above, evaluated
in-sample on the frozen all-angle model rather than on a genuinely
held-out fold; the qualitative conclusion (noise-robust,
descriptor-choice-robust, angular-sensitivity-limited) is the reportable
result, not the exact figures to the second decimal place.
...
via Bruggeman effective-medium theory \citep{bruggeman1935}, using Cu's
...
measurements for GLAD Cu and its oxides -- including two previously
unincorporated metallic-Cu series from \citet{chaffarhabilitation2020},
glass substrate, thermal-evaporation GLAD -- and against the cosine and
tangent rules. The tilt is read from the eigenvector of the smallest eigenvalue of
...
% [2026-09-09, per O.B.H.A. instruction -- widened comparison range after adding the
%  Chaffar Akkari 2020 habilitation Cu anchors (FC2020_03/04, in-house, previously
%  unused in this figure); the earlier "within ~3 deg" text no longer held once these
%  were included and is corrected here rather than left overclaiming a tighter
%  agreement than the fuller anchor set actually shows. Human author to finalise
%  wording.]
Measured this way, the ballistic model -- no physical parameter fitted to
this comparison -- reproduces the Cu /
Cu$_2$O / CuO SEM tilt to within the experimental anchors' own spread
across the range ($41^{\circ}$ vs.\ $35$--$44^{\circ}$ at $\dang=60^{\circ}$;
$54^{\circ}$ vs.\ $52$--$57^{\circ}$ at $75^{\circ}$; $64^{\circ}$ vs.\
$55$--$63^{\circ}$ at $85^{\circ}$, the low end set by one of the two
Chaffar Akkari 2020 habilitation series). It tracks the empirical cosine rule closely and lies well
...
Whether the geometric agreement reported here -- with no physical parameter
fitted to the target data -- extends across materials when only the
per-material hard-sphere radius is changed, is the subject of a companion
study \citep{companionP3}.
...
\begin{figure}[htbp]
  ...
  \caption{Column-tilt angle $\beta$ versus deposition angle $\dang$.
    \emph{Red:} the ballistic model (no physical parameter fitted to this
    comparison), tilt read from the
...
\begin{enumerate}
  ...
  \item \textbf{Rotation mode (static vs.\ helical substrate).} This is
    ...
    azimuth and restores isotropic connectivity. The original static/helical campaign used the
    $R=\SI{0.1}{nm}$ author radius; an independent re-run at the
    calibrated $R=\SI{0.128}{nm}$ Cordero radius (three seeds per arm,
    same box, height, and pitch) reproduces the effect with the same
    qualitative signature and 3/3-seed consistency -- static
    largest-component fraction $0.10$--$0.19$ with loss of
    $y$-percolation, versus $0.99$--$1.00$ two-axis-percolating for the
    paired helical runs (mean $\Delta\mathrm{LF}=-0.85$), and no
    rotation-mode effect at $75^{\circ}$ or $85^{\circ}$
    ($\Delta\mathrm{LF}=0.000$) -- so the finding does not depend on the
    uncalibrated radius of the original campaign.
\end{enumerate}

Two further candidate mechanisms remain open rather than closed. Surface
diffusion's effect on connectivity at $\dang=89^{\circ}$ has been probed
only for the helical case: a low-hop-budget surface-diffusion run at
$\dang=89^{\circ}$ leaves the film connected (largest-component fraction
$0.997\!\to\!0.956$, two-axis percolation retained), i.e.\ light
mobility does not sever an established connected topology. The
static-substrate case at $\dang=89^{\circ}$ with surface diffusion has
not been tested, and the column-tilt-focused diffusion campaign at
$\dang=85^{\circ}$ (Section~\ref{ssec:diffusion_comparison}) should not
be assumed to generalize to the connectivity observable at a different
angle without a dedicated run.
...
increases monotonically from $87.9\%$ to $98.7\%$ over the tested angle
...
$+20.7\,\mathrm{pp}$ higher; this is a cross-material, cross-method
...
\item \textbf{Data availability:} The simulation data supporting the
  findings of this study are permanently archived at Zenodo,
  DOI: \href{https://doi.org/10.5281/zenodo.22707350}{10.5281/zenodo.22707350}, mirroring the GitHub repository
  \url{https://github.com/oussamabha/ballistic-GLAD-NIF}.
...
\item \textbf{Author contribution:} O.B.H.A.\ designed and performed all
  ...
  B.G.\ co-supervised the Paris-based work and F.C.A.\ (principal supervisor)
  supervised the work overall; both critically revised
  the manuscript.
\end{itemize}
...
main text -- the void-fraction endpoints ($87.9\%$, $98.7\%$), the PCA
