...
\begin{abstract}
Glancing-angle deposition (GLAD) grows structured thin films by depositing
...
across twelve helical-rotation Cu film configurations
($\dang=60^{\circ}$--$89^{\circ}$, height $h=\SI{300}{nm}$, helical
pitch$=\SI{150}{nm}$). Void fraction (the empty-space fraction of the film)
increases monotonically from $88.1\%$ to $98.5\%$ with $\dang$, and is
highly reproducible: repeat runs with different random seeds agree to
within $0.25\%$ at this box/height convention (this reproducibility
degrades at larger lateral box size; Appendix~\ref{app:boxconv}). By
...
calculation uses the $R=\SI{0.128}{nm}$ (Cordero covalent-radius
\citep{cordero2008}) campaign
...
\citep{oskooi2010meep} was run for the single-cylinder geometry at
$\dang=65^{\circ}$ ($f=0.492$), extracting an effective index from a
thin-slab (Born-approximation) field-ratio inversion. The extraction
method was validated first against a case with a known answer (a uniform,
lossless $\varepsilon=4$ slab), recovering $n=1.907+0.134i$ against the
true $2.0+0i$ -- a residual consistent with the approximation's stated
limitation of neglecting interface reflections, not a remaining error in
the method. Applied to the same geometry and wavelength as the table
above, the FDTD-extracted extraordinary index is $n_e = 2.144+2.513i$,
compared to the Maxwell-Garnett value $n_e=2.518+1.914i$ at this row: a
$-14.8\%$ difference in the real part and $+31.3\%$ in the imaginary
part, both larger than the validation case's residual. This suggests the
Maxwell-Garnett illustration's departure from a full-wave calculation may
be non-negligible even at this single design wavelength, but the check
is a single-cylinder, single-wavelength, single-polarisation pilot with
interface reflections neglected in the extraction and no dispersion fit
away from \SI{500}{nm} -- it should be read as indicative of a real gap,
not as a validated correction to the table above. A rigorous replacement
of this screening (multi-column geometry, both polarisations, a
reflection-accounting retrieval, and a fitted material dispersion) is
future work.
...
\begin{enumerate}
  ...
  \item A separate ablation tests a different candidate mechanism for
    ...
    future work. The re-emission mechanism's
    coordination-shell cutoff factor ($k=1.15\times$ collision diameter)
    is derived and justified in Appendix~\ref{app:shellcutoff}.
