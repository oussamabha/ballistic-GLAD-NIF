...
\begin{abstract}
Glancing-angle deposition (GLAD) links a single controlled parameter, the
deposition angle $\dang$, to film porosity and to the optical anisotropy
relevant to gas-sensor and polarimetric-device performance, but the range of
structural information a ballistic simulator can predict from $\dang$ has not
been systematically established. We characterise a bead-sphere ballistic
GLAD simulator across twelve helical Cu configurations
($\dang=60^{\circ}$--$89^{\circ}$, $h=\SI{300}{nm}$, pitch$=\SI{150}{nm}$).
Void fraction increases monotonically from $88.1\%$ to $98.5\%$ with
sub-$0.25\%$ seed-to-seed variability at this box/height convention (degrading
at larger lateral box size; Appendix~\ref{app:boxconv}). A corotating-frame
oracle test shows absolute column positions carry no realization-invariant
signal recoverable by a coordinate-conditioned model, establishing this
boundary of predictive reach as a central, deliberately reported result. By
contrast, the two-point pair correlation $h_g(r;\dang)$ is reproducible and
varies smoothly with $\dang$: a compact emulator (\textsc{CorrNIF}) reproduces
held-out angles with interior leave-one-angle-out $R^2=0.992$, though not
more accurately than simple linear or PCHIP interpolation on the same folds,
and reproduces a genuinely unseen prospective angle ($\dang=72^{\circ}$,
confirmed by a dedicated simulation) with $R^2=0.9907$, again no better than
linear interpolation ($R^2=0.9909$). \textsc{CorrNIF}'s only demonstrated
advantage over these simpler baselines is exercised directly: gradient
descent through the frozen, trained model recovers the deposition angle
behind a held-out target curve from five different starting points to the
same value, matching an independent grid search to within $0.002^{\circ}$.
An illustrative aligned-cylinder
Maxwell-Garnett calculation for tungsten columns gives form birefringence
$\Delta n\sim 0.3$--$0.8$, but strong extraordinary absorption and
sensitivity to the filling-fraction definition mean no polarimetric design
window can be claimed; this is reported as a loss-aware screening
illustration, not a device prediction. The result is a simulation-only
workflow for ranking helical GLAD geometries by ensemble morphology, with the
limits of predictive reach established as an explicit finding and
quantitative optical/material calibration left to dedicated experiments.
\end{abstract}
...
across twelve helical Cu configurations spanning $\dang = 60^{\circ}$--$89^{\circ}$
...
(i)~the bead-sphere void-fraction trend and its three-seed reproducibility
...
not presented as an experimentally calibrated Cu pitch.  Twelve
configurations are simulated
for the main morphology and reproducibility study:
deposition angles $\dang \in \{60, 65, 70, 75, 80, 80, 80, 85, 85,
85, 87, 89\}^{\circ}$, where three configurations at $\dang = 80^{\circ}$
and three at $\dang = 85^{\circ}$ use independently seeded
random-number generators to assess stochastic reproducibility.
A thirteenth simulation at $\dang = 72^{\circ}$ was subsequently
performed exclusively as a prospective validation case for the
\textsc{CorrNIF} interpolation (Section~\ref{ssec:nif_prospective});
it was excluded from CorrNIF training and from all main-dataset descriptive
analyses.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lll}
    ...
    \parbox{0.34\linewidth}{Final height range (12 runs)} & \parbox{0.28\linewidth}{300.0\,nm--300.1\,nm} & \parbox{0.24\linewidth}{simulation result, this work} \\
    ...
    \parbox{0.34\linewidth}{Hard-sphere radius $R$ (diameter $2R$)} & \parbox{0.28\linewidth}{0.1\,nm (0.2\,nm)} & \parbox{0.24\linewidth}{Cu-scale model parameter (optical screening subsection uses the recalibrated $R=0.128$~nm campaign; see Section~\ref{ssec:nif_optical})} \\
    ...
    \parbox{0.34\linewidth}{Nominal deposition rate} & \parbox{0.28\linewidth}{0.2\,nm/s (dense-equiv.)} & \parbox{0.24\linewidth}{numerical setting, this work} \\
    ...
    \parbox{0.34\linewidth}{Deposited atoms per run} & \parbox{0.28\linewidth}{$1.1\times10^{7}$--$1.6\times10^{8}$} & \parbox{0.24\linewidth}{simulation result, this work} \\
    ...
    \parbox{0.34\linewidth}{Random seeds (RNG)} & \parbox{0.28\linewidth}{1 per angle; 3 at $80^{\circ},85^{\circ}$} & \parbox{0.24\linewidth}{numerical setting, this work} \\
...
The Level-A run at $\dang = 60^{\circ}$ had a corrupted final checkpoint
(active-slab resume duplication artefact, physically impossible atom count
of 163\,M vs.\ the expected ${\sim}85\,\text{M}$ at 300\,nm); both its
bead-sphere value ($\bsP = 77.2\%$) and its voxel are invalid and excluded
from all analyses.  The openness-trend curve uses $\bsP = 88.1\%$ from a
clean single-shot replacement run (literature-matched benchmark, independent
simulation from a validated starting state; SHA-256 c7509b21\ldots).
...
For each of the eleven valid voxel files we use three non-redundant
descriptors:
...
\subsection{Bead-sphere void fraction and reproducibility}
\label{sec:results_openness}

This subsection addresses the reproducibility half of \textbf{Q1} (the
...
for all twelve main configurations.  The void fraction increases
monotonically from $\bsP = \SI{88.1}{\%}$ at $\dang = 60^{\circ}$ (clean
benchmark run; Level-A primary excluded, see Section~\ref{ssec:voxel}) to
$\bsP = \SI{98.5}{\%}$ at $\dang = 89^{\circ}$, consistent with the expected
shadowing-enhancement at high obliquity.
...
\begin{figure}[htbp]
  ...
  \caption{%
    ...
    films, and this trend is highly reproducible across independent
    random seeds.
    Bead-sphere void fraction $\bsP$ (Eq.~\ref{eq:bead_sphere}), mean
    $\pm$ standard deviation across three independent random seeds, as
    a function of deposition angle for the authoritative P1 campaign
    (27 runs: nine angles $\dang \in \{60, 65, 70, 72, 75, 80, 85, 87,
    89\}^{\circ}$, three seeds each; $h = \SI{300}{nm}$, pitch
    $= \SI{150}{nm}$).
    The void fraction increases monotonically with $\dang$, reaching
    $98.5\%$ at $\dang = 89^{\circ}$; run-to-run reproducibility at
    fixed $\dang$ is discussed quantitatively in
    Table~\ref{tab:repro}.
  }
  \label{fig:openness}
\end{figure}

Three-seed reproducibility tests at $\dang = 80^{\circ}$ and $\dang = 85^{\circ}$ yield
coefficients of variation in atom areal density of
$\mathrm{CV}_{\rho_A} = 0.19\%$ and $0.23\%$, respectively
(Table~\ref{tab:repro}).  The bead-sphere void fractions at both angles
are identical to one decimal place across all three seeds, demonstrating
that the ballistic simulator exhibits excellent stochastic reproducibility.

\begin{table}[htbp]
  \centering
  \caption{%
    Three-seed reproducibility at $\dang = 80^{\circ}$ and $\dang = 85^{\circ}$.
    $\bar{P}$: mean bead-sphere void fraction;
    $\sigma_P$: standard deviation;
    $\mathrm{CV}_{\rho_A}$: coefficient of variation of atom areal density.
  }
  \label{tab:repro}
  \begin{tabular}{lccccc}
    \toprule
    $\dang$ ($^{\circ}$) & $n$ seeds & $\bar{P}$ (\%) & $\sigma_P$ (pp) &
    $\bar{\rho}_A$ (\si{nm^{-2}}) & $\mathrm{CV}_{\rho_A}$ (\%) \\
    \midrule
    80 & 3 & 93.2 & $<0.1$ & 4866 & 0.19 \\
    85 & 3 & 95.7 & $<0.1$ & 3063 & 0.23 \\
    \bottomrule
  \end{tabular}
  \par\smallskip
  \noindent\small
  $\bar\rho_A$ is the deposited-atom areal density (total atoms per unit
  substrate area, integrated over the film height): per-seed values
  $4870.9/4854.8/4871.3~\si{nm^{-2}}$ at $80^{\circ}$ and
  $3057.2/3060.1/3070.7~\si{nm^{-2}}$ at $85^{\circ}$.
  All three seeds give $\bsP$ identical to $0.1$~pp.
\end{table}
...
This is consistent with the reproducibility analysis of
Section~\ref{sec:results_openness}, where the bulk density is reproducible
to sub-percent level but the higher-order descriptors (anisotropy, width
gradient) carry more run-to-run variability.
...
\begin{enumerate}
  \item \textit{Absence of surface diffusion.}  Ballistic models
    ...
    to the ballistic-limit column shape \citep{kesapragada2006}.
...
reported here ($88.1$--$98.5\%$ over $\dang = 60$--$89^{\circ}$): the two
...
$\dang=72^{\circ}$, by $0.82^{\circ}$. This residual is consistent with,
not larger than, the interpolation error already reported for this same
target curve in Section~\ref{ssec:nif_prospective}
($R^2=0.9907$, RMSE~$=0.0186$)---the inverse recovery is limited by the same
finite-capacity, finite-training-grid interpolation error as the forward
prediction, not by a failure of the optimisation procedure itself, which is
...
$f(\dang)$ extracted from the Cu GLAD simulations.  This screening
calculation uses the $R=\SI{0.128}{nm}$ (Cordero covalent-radius) campaign
rather than the $R=\SI{0.1}{nm}$ campaign used for the void-fraction,
reproducibility, and PCA results elsewhere in this paper (Table~\ref{tab:params});
the two radii give systematically slightly different filling fractions
(void fraction $\sim 0.3$--$0.5$ percentage points lower at $R=\SI{0.128}{nm}$),
which does not change any qualitative conclusion drawn here but should not be
conflated with the primary-campaign numbers reported in
Section~\ref{sec:results_openness}.
...
\begin{enumerate}
  \item No surface diffusion: the model cannot reproduce experimental
    void fractions without calibration.
...
across twelve helical Cu configurations spanning $\dang = 60$--$89^{\circ}$
...
\textbf{Answering Q1} (void-fraction trend, reproducibility, and
comparison with experiment): the bead-sphere void fraction $\bsP$
increases monotonically from $88.1\%$ to $98.5\%$ over the tested angle
range with sub-$0.25\%$ run-to-run variability at the $\SI{100}{nm}$-box,
$h=\SI{300}{nm}$ convention (Appendix~\ref{app:boxconv} shows this
reproducibility is convention-dependent, degrading at larger lateral box
size), confirming that the bulk void fraction is well-defined and
reproducible at the descriptor level within that convention.
...
main text -- the void-fraction endpoints ($88.1\%$, $98.5\%$), the PCA
