...
\author[3]{\fnm{Nicolas} \sur{Martin}}
...
\affil[3]{%
  \orgdiv{Institut FEMTO-ST},
  \orgname{CNRS, Universit\'e de Franche-Comt\'e},
  \orgaddress{\city{Besan\c{c}on}, \country{France}}%
}
...
\abstract{Glancing-angle deposition (GLAD) links a single controlled parameter, the
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{0.36\linewidth}p{0.28\linewidth}p{0.27\linewidth}}
    ...
    Final height range (12 runs) & \SI{300.0}{nm}--\SI{300.1}{nm} & simulation result, this work \\
    ...
    Hard-sphere radius $R$ (diameter $2R$) & \SI{0.1}{nm} (\SI{0.2}{nm}) & Cu-scale model parameter (optical screening subsection uses the recalibrated $R=\SI{0.128}{nm}$ campaign; see Section~\ref{ssec:nif_optical}) \\
    ...
    Nominal deposition rate & \SI{0.2}{nm/s} (dense-equiv.) & numerical setting, this work \\
    ...
    Deposited atoms per run & $1.1\times10^{7}$--$1.6\times10^{8}$ & simulation result, this work \\
    Stopping criterion & target height $h=\SI{300}{nm}$ reached & prescribed in this work \\
    Random seeds (RNG) & 1 per angle; 3 at $80^{\circ},85^{\circ}$ & numerical setting, this work \\
