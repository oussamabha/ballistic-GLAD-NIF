...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{P4_neb_hop_barrier}
\caption{Minimum-energy path for the Cu(100) adatom hop on the Mishin
\textit{et al.}~\cite{mishin2001} EAM potential (7-image NEB, this work),
with the three literature comparison barriers overlaid.}
\label{fig:neb}
\end{figure}

The minimum-energy path is shown in Fig.~\ref{fig:neb}. The resulting
diffusion length is
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{P4_geometry_kinetics_decomposition}
\caption{Geometry--kinetics decomposition at the two height-matched
deposition angles (\ang{82} vs.\ the nearest \ang{80} ballistic reference,
2\si{\degree} offset; \ang{89}, matched exactly), diffusion-off vs.\
diffusion-on at dwell-hop cap 1. Both points show the same signature: film
stays fully connected, mass increases (diffusion fills in rather than
disperses), and interpenetration does not worsen.}
\label{fig:decomp}
\end{figure}

The geometry--kinetics decomposition at the two height-matched deposition
angles (Fig.~\ref{fig:decomp}) shows the diffusion kernel at dwell-hop cap 1
produces a bounded, single-parameter correction. At \ang{89} (the exactly
matched point) the largest-component fraction goes from \num{0.997} to
\num{0.956} (both connected, both percolating in both directions), atom
count rises \SI{14}{\percent} (diffusion fills in, it does not disperse),
interpenetration does not worsen, and column tilt is unchanged to within
\SI{1}{\degree}. At \ang{82} (vs.\ the nearest \ang{80} ballistic reference,
\SI{2}{\degree} offset) the same pattern holds: connectivity stays at
\num{0.99998}, atom count rises \SI{22}{\percent}, and interpenetration
falls rather than worsens. Lower deposition angles are directionally
...
A separate short-timescale molecular-dynamics deposition test (LAMMPS,
\num{2000} Langevin relaxation steps per atom, a \num{216}-atom Cu(100)
slab seed, $\alpha=\ang{85}$) measures mechanical settling of a landed atom into
the nearest favourable site, on a picosecond window: \num{49}/\num{50}
sequentially deposited atoms mechanically join the main cluster within
their relaxation window. That is a different quantity from thermally
activated surface diffusion, which needs milliseconds to seconds of real
time to accumulate the \num{1e3}--\num{1e4} hops behind $L_D$. The two
should not be combined into a single calibrated mobility parameter.
...
\item \textbf{Code availability:} The NEB, diffusion-length, and
  ...
  this study are permanently archived at Zenodo, DOI: \href{https://doi.org/10.5281/zenodo.22707350}{10.5281/zenodo.22707350},
