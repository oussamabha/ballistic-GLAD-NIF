...
%      n=231 rows / 43 materials (grown 2026-08-20: Ta2O5 beta series,
%      TiN second-source rows): gradient-boosted trees +49.7% RMSE
%      vs. tangent-rule physics baseline, permutation p=0.20 (n_perm=100).
%      RMSE bar cleared, significance bar NOT cleared.
%    - Porosity(alpha), n=104/22 (grown 2026-08-20: Nb2O5 packing-density
%      series): Gaussian process +18.8% RMSE vs naive-mean baseline,
%      p=0.32. NOT cleared.
...
class-stratified fit of Section~\ref{ssec:stratification}).
...
snapshot ($\beta(\alpha)$: $n=231$ rows/43 materials;
porosity($\alpha$): $n=104/22$; refractive index: $n=78/13$; $E_g$:
$n=67/10$). For $\beta(\alpha)$, the tangent-rule baseline achieves a
\lomo{} RMSE of 26.368$^{\circ}$; every learned model outperforms it,
with gradient-boosted trees the best performer at 13.255$^{\circ}$
($+49.7\%$). This comfortably clears the pre-registered 15\% RMSE bar.
The material-level permutation test, however, returns $p=0.20$
($n_{\mathrm{perm}}=100$), far from the 0.05 significance threshold.
For porosity($\alpha$), Gaussian process reaches $+18.8\%$ RMSE
improvement over the naive-mean baseline (25.081 vs. 30.893,
porosity in percentage points) -- clearing the RMSE bar -- with
permutation $p=0.32$. One of the five porosity candidates, linear
regression, underperforms the baseline outright ($-26.8\%$); the
remaining three learned models cluster closely together and all
clear the RMSE bar (Gaussian process $+18.8\%$, the best performer;
gradient-boosted trees $+17.9\%$; random forest $+18.4\%$),
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrlrrc}
    ...
    \midrule
    $\beta(\alpha)$          & 231 & 43 & gradient-boosted trees & $+49.7\%$ & 0.20 & No \\
    Porosity $\phi(\alpha)$  & 104 & 22 & Gaussian process       & $+18.8\%$ & 0.32 & No \\
...
$\beta(\alpha)$ ($p=0.20$), porosity($\alpha$) ($p=0.32$), and
$n(\alpha)$ ($p=0.54$) -- for which the independence assumption is
only approximate (materials and source papers overlap across
observable corpora); this gives $X^2=6.73$, $df=6$, $p=0.347$, also
not cleared.
