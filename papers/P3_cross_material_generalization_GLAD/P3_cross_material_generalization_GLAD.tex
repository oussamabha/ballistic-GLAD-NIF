...
%  (revtex4-1, JVST-A target); see ../article_draft_AIP/P3_cross_material_generalization_GLAD_AIP.tex
...
%
%  Target: methodological / materials-informatics venue
%          (companion to P1 ballistic-simulation-scope and P2
%          density-calibrated-NIF manuscripts)
%
...
%      superseded trace.
%
%  Draft status: first LaTeX draft, assembled 2026-08-20 from
%    02_PINN_NIF/P3_ML/P3_RESULTS_SECTION_DRAFT_20260815.md (primary
%    source, "FINAL RESULT v2" section) and its companion reports
%    (material_class_stratified_analysis_20260817.md,
%    bayesian_hierarchical_report.md, ranking_reformulation_report.md,
%    feature_importance_report.md, model_comparison_report.md,
%    feature_table_report_v2.md, band_gap_eg_report.md,
%    refractive_index_phase0_report.md). No numbers invented; every
%    figure below traces to one of those files.
...
\title{%
  Cross-Material Transfer of GLAD Column-Tilt and Porosity
...
$\beta(\alpha)$ at an earlier, smaller corpus snapshot; $p=0.10$--$0.86$
across every variant tested at the current snapshot, with the pooled
$\beta(\alpha)$ and porosity($\alpha$) fits -- $p=0.10$ and $p=0.12$ --
now the closest either has come to significance across roughly eleven
corpus-growth rounds). We report this
...
with it: a diffuse, imprecisely-localized effect of the size the
Bayesian $R^2$ posteriors suggest is exactly what a permutation test at
$n=14$--$46$ materials is underpowered to detect at $p<0.05$. We
present this paper as a resource for the
...
\begin{table}[htbp]
  ...
  \caption{Best-model \lomo{}-CV comparison against the pre-registered
    bar, all four target observables, latest corpus
...
$n=46$ materials, $p=0.10$) -- a natural hypothesis
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrlrrc}
    ...
    \midrule
    \emph{Pooled (all classes)} & 46 & 250\footnotemark & gradient-boosted trees & $+50.0\%$ & 0.10 & No \\
    \bottomrule
  \end{tabular}
\end{table}
\footnotetext{Row count for the pooled reference row reflects the
  current Table~\ref{tab:pooled_all} snapshot ($n=46$ materials, 250
...
Six population-level coefficients are reported jointly rather than
selected post hoc, and at the current corpus snapshot none excludes
...
to 46 materials, three pre-registered material-class subgroups, two
