...
one crossing 0.05 alone. That is not the regime observed here: input
p-values of 0.20--0.86 are broadly consistent with genuinely null or
near-null per-test effects, not a diffuse signal masked only by
underpowering. This result therefore argues, if anything, against
per-test underpowering as the sole explanation for the negative
result at the subgroup/observable level -- while remaining fully
...
tests, and also ruled out (combined $p=0.715$ and $p=0.390$
respectively; neither clears the bar, and the input p-value magnitudes
are not consistent with the diffuse-weak-signal regime Fisher's method
is designed to detect). The specific, remaining, and
