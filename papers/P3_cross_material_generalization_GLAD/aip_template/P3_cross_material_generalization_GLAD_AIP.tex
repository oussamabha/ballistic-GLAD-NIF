...
\begin{abstract}
Glancing-angle deposition (GLAD) produces tilted, porous columnar films whose column-tilt $\beta$ and porosity $\phi$ set their optical and electrical anisotropy. Predictive relations for both are either purely geometric (the tangent rule for
$\beta$) or material-specific fits; none uses material identity -- via
descriptors such as atomic mass, melting point, bulk density -- to predict a
material never seen during fitting. We assembled a literature-derived corpus of measured $\beta(\alpha)$
and porosity($\alpha$) spanning up to $40$ materials (elemental metals, oxides,
fluorides, chalcogenides), fitted four models under leave-one-material-out cross-validation, and pre-registered a strict acceptance
bar before inspecting any result: $\geq 15\%$ RMSE improvement over a
physics/naive baseline \emph{and} a material-level permutation-test $p < 0.05$.
Every learned model clears the RMSE bar (up to $+54.4\%$ for $\beta$, $+28.5\%$
for porosity), but no configuration clears the permutation bar -- not the
pooled fit, three pre-registered material-class subgroups, two targeted
corpus-growth interventions, a pairwise-ranking reformulation, or a
Fisher's-method combination ($p = 0.715$); the best permutation $p$ is $0.09$
at an earlier, smaller corpus snapshot and $0.20$--$0.36$ across every current
variant. This is a thoroughly instrumented negative result: the binding constraint is
material-level sample size, not model, feature, task formulation, or extraction
quality, each tested and ruled out. A complementary pre-registered Bayesian hierarchical model ($n = 250$ rows,
$46$ materials) finds no descriptor coefficient's $94\%$ highest-density
interval excludes zero, yet places $73$--$84\%$ posterior probability on
descriptors explaining more than $10\%$ of between-material variance -- a
diffuse effect a permutation test at this corpus size is underpowered to
detect, consistent with the frequentist verdict. The paper maps where the statistical-power boundary for this problem currently
sits and identifies the corpus-growth target ($30+$ elemental metals,
especially platinum-group and rare-earth species) that would most directly move it.
\end{abstract}
...
shuffled $n_{\mathrm{perm}}=100$ times (fixed RNG seed $20260815$
throughout this paper's LOMO/permutation pipeline, for exact
reproducibility), the same \lomo{} model-fitting
...
rows). Table~\ref{tab:corpus_growth} reports the resulting best figures at
this campaign's 2026-08-16 checkpoint ($n=132$ rows/27 materials for
$\beta(\alpha)$) -- an intermediate snapshot in the corpus's growth, kept
here for its own sake as the direct before/after comparison against the
$n=73$--$93$ starting point discussed above, and distinct from the larger,
later corpus snapshot already reported in
Table~\ref{tab:pooled_all}.

A separate, more targeted test isolates \emph{which} physical predictor is
doing the residual work rather than adding radius and $T/T_{\mathrm{melt}}$
together to a multi-feature model. A one-parameter constrained form,
$\beta_{\mathrm{pred}}(\alpha) = \beta_{\mathrm{tangent}}(\alpha) +
c\,X(\mathrm{material})\,g(\alpha)$, was fit separately for each of four
candidate predictors $X$ under the identical \lomo{} + material-identity
permutation protocol, restricted to the elemental-metal subset
($n=9$) to isolate a single chemical class. Homologous temperature
$T_s/T_m$ -- the predictor motivated by a residual room-temperature-mobility
hypothesis -- clears the RMSE bar (+25.3\%) but its permutation $p=0.71$
(and a companion T$_m$-shuffle test on the full corpus gives $p=0.89$),
i.e.\ it does no real material-identity work despite improving RMSE. The
covalent atomic radius, by contrast, is the only one of the four predictors
that approaches significance (+36.7\% RMSE, $p=0.205$), an order of
magnitude closer to the pre-registered $p<0.05$ bar than any thermal or
kinetic predictor tested (mass scaling $p=0.90$, cohesive-energy scaling
$p=0.84$). This converges with an independent, disjoint line of evidence: a
controlled ballistic simulation across 14 elemental metals (companion
paper) finds the deposition-angle-independent tangent-rule offset
correlates with covalent radius at $r=0.57$ ($p=0.032$, $n=14$), while the
same simulation's test for a $T_s/T_m$ dependence fails
($|r|=0.41$, $p=0.16$). Two independent methods -- a literature
meta-analysis and a from-scratch controlled simulation -- agree that
material identity in GLAD morphology enters through the atomic length
scale, not through a thermal-mobility channel, even though neither method
alone clears full statistical significance.

\begin{table}[htbp]
  \centering
  \caption{Corpus-growth campaign result, all four observables, at the
    2026-08-16 mid-campaign checkpoint ($\beta(\alpha)$: $n=132$
    rows/27 materials; the single most effective lever tried at this
    checkpoint was the deep figure-level re-digitization pass). This is
    an intermediate, dated snapshot illustrating the growth trajectory,
    not the paper's final/largest corpus -- see
    Table~\ref{tab:pooled_all} for the latest snapshot of all four
    observables.}
...
documents referenced in this paper are permanently archived at Zenodo,
DOI: \href{https://doi.org/10.5281/zenodo.22707350}{10.5281/zenodo.22707350}, mirroring the GitHub repository
\url{https://github.com/oussamabha/ballistic-GLAD-NIF}; the corpus is drawn from
